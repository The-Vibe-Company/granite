...
5. Save `review/brief.md`, preview/rule JSON and `review/scope.json` under the already ignored Ship PR run directory. The scope artifact contains refs, intended files, hashes, unrelated files, exclusions and requested focus areas. Avoid collecting secret values into artifacts. On failed setup, invalid JSON or unresolved scope, repair it before review or report a concrete blocker; zero output is not a clean review.
...
Goal: <user objective, non-goals and acceptance checks>
